...
import { ChildToParent, Diff, PatternQuery, SearchQuery, SgSearch, YAMLQuery } from '../types';
...
function locationToRange(locations: ChildToParent['previewDiff']['locationsToSelect']) {
...
async function doPreview(fileUri: Uri, { filePath, locationsToSelect }: ChildToParent['previewDiff']) {
  ...
  await commands.executeCommand('vscode.diff', fileUri, previewUri, `${filename} ↔ ${filename} (Replace Preview)`, {
    preserveFocus: true,
...
function bufferMaker(bytes: Uint8Array) {
  ...
  function receiveResult(replace: string, byteOffset: { start: number; end: number }) {
