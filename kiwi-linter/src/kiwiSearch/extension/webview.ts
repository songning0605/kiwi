...
export function activateWebview(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    window.registerWebviewViewProvider(SearchSidebarProvider.viewType, provider, {
      webviewOptions: { retainContextWhenHidden: true }
    }),
    commands.registerCommand('vscode-i18n-linter.refreshSearch', refreshSearch),
    commands.registerCommand('vscode-i18n-linter.clearSearchResults', clearSearchResults),
    commands.registerCommand('vscode-i18n-linter.expandAll', toggleAllSearch),
    commands.registerCommand('vscode-i18n-linter.collapseAll', toggleAllSearch)
  );
}
...
function setupParentPort(webviewView: vscode.WebviewView) {
  ...
  parentPort.onMessage('reload', _payload => {
    ...
    webviewView.webview.html = webviewView.webview.html.replace(/(nonce="\w+?")|(nonce-\w+?)/g, `nonce="${nonce}"`);
  });
}

class SearchSidebarProvider implements vscode.WebviewViewProvider {
  public static readonly viewType = 'kiwi.search.input';
  ...
  private getHtmlForWebview(webview: vscode.Webview) {
    ...
    const stylexUri = webview.asWebviewUri(vscode.Uri.joinPath(this._extensionUri, 'out', 'webview.css'));

    const stylesResetUri = webview.asWebviewUri(vscode.Uri.joinPath(this._extensionUri, 'media', 'reset.css'));
    const stylesMainUri = webview.asWebviewUri(vscode.Uri.joinPath(this._extensionUri, 'media', 'vscode.css'));
    const iconsUri = webview.asWebviewUri(vscode.Uri.joinPath(this._extensionUri, 'icons'));
...
parentPort.onMessage('search', async () => {
  ...
  vscode.commands.executeCommand('setContext', 'vscode-i18n-linter.searchDefaultCollapse', false);
});

function toggleAllSearch() {
  parentPort.postMessage('toggleAllSearch', {});
  defaultCollapse = !defaultCollapse;
  vscode.commands.executeCommand('setContext', 'vscode-i18n-linter.searchDefaultCollapse', defaultCollapse);
}
