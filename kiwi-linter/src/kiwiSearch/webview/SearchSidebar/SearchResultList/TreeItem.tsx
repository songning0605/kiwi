...
const TreeItem = ({ className, langKey, matches }: TreeItemProps) => {
  ...
  const props = stylex.props(styles.treeItem, isTreeActive && styles.activeIndent);
  ...
  return (
    <div className={`${className} ${props.className}`} style={props.style}>
      ...
      <ul style={{ display: isExpanded ? '' : 'none' }}>
        <MatchList langKey={langKey} matches={matches} />
      </ul>
    </div>
  );
};
